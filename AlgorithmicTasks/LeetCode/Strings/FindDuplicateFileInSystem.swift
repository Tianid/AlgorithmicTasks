    ...
    private func findDuplicate(_ paths: [String]) -> [[String]] {
        ...
        for item in paths {
            ...
            for i in 1..<arr.count {
                let fileParams = arr[i].split(separator: "(")
                guard let fileName = fileParams.first else { continue }
                guard let fileContent = fileParams.last else { continue }
                
                let formatedString = "\(dir)/\(fileName)"
                
                
                dict[String(fileContent), default: []].append(String(formatedString))
            }
        }
